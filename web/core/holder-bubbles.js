...
export function moveHolderBubble(model, node, x, y, vx = 0, vy = 0) {
  ...
  /* One solve, with the grabbed node pinned, so neighbours visibly yield
     during the gesture rather than popping away after release. This used to
     run TWICE and to be called at pointer rate, which on a high-frequency
     pointer meant several O(n^2) passes per frame. The caller now coalesces
     to one call per frame; this stays a pure function so its test can drive
     it directly. */
...
export function createHolderBubbleMap(canvas, options = {}) {
  ...
  const applyDrag = () => {
    dragFrame = null;
    if (!model || !down || !down.node || !down.dragging) return;
    const now = performance.now();
    const elapsed = clamp(now - down.lastAt, 8, 48);
    const x = down.pendingX;
    const y = down.pendingY;
  ...
  const onPointerMove = (event) => {
    ...
    if (down && down.pointerId === event.pointerId && down.node) {
      ...
      if (down.dragging) {
        ...
        /* Coalesce: keep only the newest position and settle it once per
           frame. Pointer events can arrive several times per frame, and
           solving on each one is wasted work the display never shows. */
        down.pendingX = point.x - down.offsetX;
        down.pendingY = point.y - down.offsetY;
        if (dragFrame == null) dragFrame = requestAnimationFrame(applyDrag);
      }
      return;
    }
