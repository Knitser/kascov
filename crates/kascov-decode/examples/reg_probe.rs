...
fn main() {
    ...
    for path in std::env::args().skip(1) {
        ...
        println!(
            "   decoder={} template={:?} fields={}",
            d.decoder,
            d.template,
            d.fields.len()
        );
        for f in &d.fields {
            let v = if f.value.len() > 40 {
                format!("{}…({}B)", hex::encode(&f.value[..40]), f.value.len())
            } else {
                hex::encode(&f.value)
            };
            println!("     - {} = {}", f.name, v);
        ...
        println!(
            "   kcc20.revealed_template = {:?}",
            kcc20::revealed_template(&reg, 0, &bytes)
        );
        ...
        match kcc20::decode_token_state(&reg, 0, &bytes) {
            Some(s) => println!(
                "   token_state owner={} type={} amount={:?} minter={:?}",
                hex::encode(s.owner),
                s.identifier_type,
                s.amount_i64(),
                s.is_minter()
