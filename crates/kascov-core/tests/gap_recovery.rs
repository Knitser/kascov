...
fn valid_genesis_id(outpoint: Outpoint, n_outputs: u32) -> CovenantId {
    let fields: Vec<(u32, u64, u16, &[u8])> = (0..n_outputs)
        .map(|i| (i, VALUE, 0, SPK.as_slice()))
        .collect();
...
impl ChainSource for FakeGapChain {
    ...
    async fn virtual_chain_from(&self, cursor: BlockHash) -> Result<ChainStep> {
        Ok(self.steps.get(&cursor).cloned().unwrap_or(ChainStep {
            removed: vec![],
            added: vec![],
        }))
    }
...
async fn gap_recovery_merges_canonical_history_and_is_idempotent() {
    ...
    chain.steps.insert(
        ...
        ChainStep {
            removed: vec![],
            added: vec![accepted(h(0x01), &[tx_id(0xA0)])],
        },
    );
    sync_once(&chain, &mut store, Some(h(0x00)), |_| {})
        .await
        .unwrap();
    ...
    let report = recover_gap(&chain, &mut store, &GapRecoveryOptions::default(), |_| {})
        .await
        .unwrap();
    assert_eq!(
        store.gap_recovery_pending().unwrap(),
        None,
        "in-flight marker must retire"
    );
    ...
    let y_view: Vec<(u64, String, TxId)> = store
        .events(&cov_y)
        .unwrap()
        .iter()
        .map(|e| (e.seq, e.kind.clone(), e.txid))
        .collect();
    assert_eq!(
        y_view,
        [
            (0, "genesis".into(), tx_id(0xD0)),
            (1, "burn".into(), tx_id(0xE0))
        ]
    );
    ...
    let z_view: Vec<(u64, String, TxId)> = store
        .events(&cov_z)
        .unwrap()
        .iter()
        .map(|e| (e.seq, e.kind.clone(), e.txid))
    ...
    assert_eq!(
        z_view,
        [
            (0, "genesis".into(), tx_id(0xF0)),
            (1, "burn".into(), tx_id(0x9A))
        ]
    );
    ...
    let w_view: Vec<(u64, String, TxId)> = store
        .events(&cov_w)
        .unwrap()
        .iter()
        .map(|e| (e.seq, e.kind.clone(), e.txid))
    ...
    assert!(
        w_sum.lineage_complete,
        "gap genesis must complete the lineage"
    );
    ...
    let w_cell = |txid: TxId, index: u32| {
        w_utxos
            .iter()
            .find(|u| u.outpoint.txid == txid && u.outpoint.index == index)
            .unwrap()
    };
    ...
    let again = recover_gap(&chain, &mut store, &GapRecoveryOptions::default(), |_| {})
        .await
        .unwrap();
    ...
    let opts = GapRecoveryOptions {
        from_daa: Some(150),
        to_daa: Some(1_500_000),
        ..Default::default()
    };
    assert!(
        recover_gap(&chain, &mut store, &opts, |_| {})
            .await
            .unwrap()
            .already_recovered
    );
    // Byte-identical outcome: same events, same seqs, same spends.
    let x_after: Vec<(u64, String, TxId)> = store
        .events(&cov_x)
        .unwrap()
        .iter()
        .map(|e| (e.seq, e.kind.clone(), e.txid))
