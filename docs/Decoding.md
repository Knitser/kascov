...
For each state script:

1. disassemble and retain the raw facts;
2. try fixed/observed template skeletons;
3. try SilverScript skeletons;
4. try standard P2PK and P2SH state shapes;
5. otherwise return the disassembly with no template label.

Recognition is deterministic and fail-closed. Friendly names, token metadata, or surrounding transaction context never cause a script match.

For a spent P2SH state, the committed state and revealed program are decoded separately. A valid reveal can name the program even though the state itself remains a `p2sh commitment`.

## KCC-1 primitives

`kcc1.rs` implements the draft's byte-level conventions:

- explicit push encoding/decoding;
- canonical state and argument integers;
- four-byte dispatch tags derived from signatures;
- template hashes from fixed prefix/suffix material;
- P2SH envelope and signature-script construction;
- typed state encoding/decoding;
- packed state commitments and verification.

These are protocol primitives, not UI helpers. Conformance vectors live in unit tests; vocabulary can change while KCC-1 remains a draft.

## KCC20 decoding and proof

`kcc20.rs` separates parsing from proof:

- `decode_token_state` extracts a candidate state using the registry;
- `kcc1_template_hash` identifies the canonical template;
- `splice_token_state` reconstructs state-bearing programs;
- `prove_output_state` checks that reconstructed bytes match the on-chain output commitment.

The accounting layer in `kascov-core` replays proven states into [[Storage Schema#Deterministic derived tables]]. A decoded token is not automatically a verified token. Verification requires every relevant event and live balance cell to satisfy known transition rules and commitment checks.

## Observed program families

`observed.rs` contains skeletons derived from real on-chain program families, including repeated structures. They are evidence-backed recognizers: a program must match the fixed instruction body and slot constraints. New observed families should ship with fixtures or known hashes and a negative test against close shapes.

## Decoder invariants

- Raw hex and ASM remain available even when no label matches.
- A label never changes chain classification.
- A reveal is accepted only when cryptographically bound to the spent state.
- Repeated constructor slots must agree.
- Heuristics are surfaced as heuristics.
- Rust and browser decoding must stay semantically aligned.

See [[Testing Strategy#Decoder testing]].
